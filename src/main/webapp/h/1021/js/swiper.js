...
var Swiper = function (selector, params) {
    ...
    var defaults = {
        ...
        freeModeFluid : false,
        momentumRatio: 1,
        momentumBounce: false,//false时禁用自由模式下的（free mode）动量弹性
        momentumBounceRatio: 1,
        slidesPerView : 1,
        slidesPerGroup : 1,
        slidesPerViewFit: true, //Fit to slide when spv "auto" and slides larger than container
        simulateTouch : true,//接受鼠标事件时与触屏事件相似。
        followFinger : true,//值为false时，仅当你释放slide时才会滑动，当你用手指按住滑块它不会动。
        shortSwipes : true,//值为false时，禁用short swipes
        longSwipesRatio: 0.5,
        moveStartThreshold: false,
        onlyExternal : false,
        createPagination : true,//pagination span
        pagination : false,
        paginationElement: 'span',
        paginationClickable: false,
        paginationAsRange: true,//为真时，跟可见块相关的指示器按钮会新增css类。当使用slidesPerview超过1时会十分有用。
        resistance : '100%', // or false or 100% 值为false时禁用resistant bounds（抵抗反弹）,设置为‘100%'时启用resistant（抵抗）模式
        scrollContainer : false,
        preventLinks : true,//禁止<a>标签链接
        ...
        // Autoplay
        autoplay: false,
        autoplayDisableOnInteraction: true,//当设置为false时，用户操作之后（swipes,arrow以及pagination 点击）autoplay不会被禁掉
